...
function removeFireMark(source: string) {
  return source.replace(
    `  vec3 markCol = mix(vec3(1.0, 0.92, 0.78), vec3(1.0, 1.0, 0.98), core);
  col = mix(col, markCol, inside);
  col += exp(-max(d0, 0.0) / 0.026) * vec3(1.0, 0.55, 0.22) * 0.30 * (1.0 - inside * 0.92);
  col += exp(-max(d0, 0.0) / 0.09)  * vec3(0.55, 0.14, 0.03) * 0.28 * (0.55 + 0.45 * n2);`,
    `  // The host intentionally omits the source mark fill and mark halo.`,
  ).replace(
    `new Panel(document.querySelector('[data-fx="fire"]'), FRAG_FIRE, logos.claude, {`,
    `new Panel(document.querySelector('[data-fx="fire"]'), FRAG_FIRE, buildLogo("M0 21 H24 V24 H0 Z"), {`,
  );
}
...
(function () {
  ...
  const detailedSource = applyDetailPatches(elementalMarksSource);

  return (variant === "fire" ? removeFireMark(detailedSource) : detailedSource)
